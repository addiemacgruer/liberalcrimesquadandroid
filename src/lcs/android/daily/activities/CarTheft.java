  ...
  private static boolean stealcar(final Creature cr) {
    ...
    do {
      ...
      if (alarmon) {
        ...
        ui().text(":   ").add();
        if (sensealarm) {
          ui().text("REMOVE YOURSELF FROM THE VEHICLE!   <BEEP!!> <BEEP!!>").color(Color.RED).add();
        } else {
      ...
      if (!sensealarm) {
        ...
        ui(R.id.gcontrol).button('x').text("The Viper has finally deterred " + cr.toString() + ".")
            .add();
      }
      int method = 0;
      do {
      ...
      if (method == 0) {
        if (cr.skill().skillCheck(Skill.SECURITY, CheckDifficulty.CHALLENGING)) {
          ...
          case 3:
            ui().text(
                cr.toString() + " makes something in the engine click, but the car doesn't start.")
                .add();
            break;
          case 4:
            ui().text(
                cr.toString() + " manages to turn on some dash lights, but the car doesn't start.")
                .add();
            break;
          }
        }
    ...
    if (chaselev > 0 || v.type == Game.type.vehicle.get("POLICECAR") && i.rng.chance(2)) // Identify
    // police
    // cruiser.
